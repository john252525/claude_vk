...
| Маршрут | Описание |
|---|---|
| `GET /login` | Страница входа (ввод секретного ключа) |
...
3. Добавьте переменные:
   ...
   - `APP_SECRET` = любой секретный ключ для входа в веб-интерфейс (придумайте сами)
...
Откройте ваш URL — вы увидите страницу логина. Введите ваш `APP_SECRET` для доступа.
...
railway variables set APP_SECRET=ваш_секретный_ключ
...
# 1. Создайте .env файл
cp .env.example .env
# Заполните VK_ACCESS_TOKEN, VK_GROUP_ID и APP_SECRET в .env
...
- **Авторизация**: Веб-интерфейс защищён секретным ключом (`APP_SECRET`). При первом входе вводите ключ — далее сессия хранится в cookie 30 дней. Для JSON API можно передавать `?key=YOUR_APP_SECRET` в query-параметре.
